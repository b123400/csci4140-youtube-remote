...
PlayList.prototype = {
	...
	,reloadElement: function() {
		...
		var clearButton = window.document.createElement("button");
...
Control.applyCommand = function(command,option) {
...
Control.init = function(global) {
	...
	var wrapper = window.document.getElementsByClassName("player-wrapper")[0];
	...
	var qrcode;
	qrcode = js_Boot.__cast(window.document.createElement("img") , HTMLImageElement);
	qrcode.src = "https://chart.googleapis.com/chart?cht=qr&chs=200x200&chl=" + window.location.href;
...
Button.prototype = {
	...
	,prependElement: function(tagName,className) {
...
Video.prototype = {
	play: function() {
		if(Control.isDesktopMode()) YoutubeAPI.shared.play(this.videoID); else {
			var client = window.client;
			client.broadcastCommand("play",this.videoID);
		}
	}
	...
	,makeElementContent: function() {
		...
		if(this.closeButtonElement == null) {
			this.closeButtonElement = window.document.createElement("div");
			this.closeButtonElement.innerHTML = "x";
			this.closeButtonElement.className = "close-button";
			this.closeButtonElement.addEventListener("click",function(e) {
				e.stopPropagation();
				Control.playlist.removeVideo(_g);
			});
		}
